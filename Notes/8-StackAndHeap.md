...
​有些实现的确会进行错误检查：它会记录下已分配的`void*`指针。并且通过将传入的参数与保存下来的`void*`对比进行快速检查。以确保传入的地址是记录下`void*`中的一个。在debug模式下，如果地址不合法，程序会报错。如果在release模式下地址不合法，编译器会忽略错误并且不会释放这块内存，因为这样做的话之后会出问题。
